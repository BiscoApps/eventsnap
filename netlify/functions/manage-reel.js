...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!action || !eventCode) {
      return withCors({ statusCode: 400, body: JSON.stringify({ error: 'Missing action or eventCode' }) });
    }

    if (action === 'create') {
      if (!title) return withCors({ statusCode: 400, body: JSON.stringify({ error: 'Missing title' }) });
      ...
      if (error) {
        ...
        return withCors({ statusCode: 500, body: JSON.stringify({ error: 'Failed to create reel' }) });
      }
      return withCors({ statusCode: 200, body: JSON.stringify({ reel: data }) });
    }

    if (action === 'update') {
      if (!reelId) return withCors({ statusCode: 400, body: JSON.stringify({ error: 'Missing reelId' }) });
      const updates = {};
      ...
      if (error) {
        ...
        return withCors({ statusCode: 500, body: JSON.stringify({ error: 'Failed to update reel' }) });
      }
    ...
    if (action === 'delete') {
      ...
      if (error) {
        ...
        return withCors({ statusCode: 500, body: JSON.stringify({ error: 'Failed to delete reel' }) });
      }
      return withCors({ statusCode: 200, body: JSON.stringify({ success: true }) });
    }

    return withCors({ statusCode: 400, body: JSON.stringify({ error: 'Unknown action' }) });
  } catch (err) {
